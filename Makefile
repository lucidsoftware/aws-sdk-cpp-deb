...
debian/%-dev/DEBIAN/control: debian/$$*/usr/lib/$$*.so debian/control $$(SYMBOLS)
	@mkdir -p $(@D)
	echo Architecture: $$(dpkg --print-architecture) > $@
# only libaws because other libs don't follow a pattern for their source packages
	echo Depends: '$* (= $(VERSION)), ' $$(dpkg-shlibdeps -O -xlibstdc++6 --warnings=0 $< | sed s/shlibs:Depends=// | tr , $$'\n' | grep libaws | tr $$'\n' ,) | sed 's/, *$$//' >> $@
